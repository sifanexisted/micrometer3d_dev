...
def get_nd_sincos_pos_embed(embed_dim, grid_size):
    def get_nd_sincos_pos_embed_from_grid(embed_dim, grid):
        assert embed_dim % grid.shape[0] == 0
        emb_list = []
        for i in range(grid.shape[0]):
            emb = get_1d_sincos_pos_embed_from_grid(
                embed_dim // grid.shape[0], grid[i]
            )
            emb_list.append(emb)

        emb = jnp.concatenate(emb_list, axis=-1)
        return emb

    n_dim = len(grid_size)
    assert (embed_dim % (2 * n_dim) == 0)

    grid = jnp.meshgrid(*[jnp.arange(size, dtype=jnp.float32) for size in grid_size], indexing="ij")
    grid = jnp.stack(grid, axis=0)
    grid = grid.reshape(n_dim, 1, *grid_size)
    pos_embed = get_nd_sincos_pos_embed_from_grid(embed_dim, grid)
...
pos_emb_init = get_nd_sincos_pos_embed
...
class Encoder(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = PatchEmbed3D(self.patch_size, self.emb_dim)(x)
        ...
        pos_emb = self.variable(
            ...
            pos_emb_init,
            self.emb_dim,
            (h // self.patch_size[0], w // self.patch_size[1], d // self.patch_size[2]),
        )
